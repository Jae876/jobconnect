...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/auth/register/job-seeker", async (req, res) => {
    try {
      ...
      await storage.createJobSeeker({
        ...
        yearsExperience: parseInt(validatedData.yearsExperience),
  ...
  app.post("/api/reviews", requireAuth, async (req, res) => {
    try {
      ...
      const validatedData = companyReviewSchema.parse({
        ...req.body,
        jobSeekerId: jobSeeker.id,
      });

      const review = await storage.createCompanyReview(validatedData);
      res.json(review);
    } catch (error: any) {
